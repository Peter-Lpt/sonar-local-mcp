...
7. **统一配置文件**：配置集中在仓库根 `sonar-local-config.json`（或 `SONAR_CONFIG` 指定的 JSON），含 `sonar_java` / `timeout_seconds` / `max_text_chars` / `severity` / `min_severity` / `sonarqube.{url,token,profile,project}`；优先级 = 环境变量 > 配置文件 > 默认值。真实配置含 token,勿提交。
